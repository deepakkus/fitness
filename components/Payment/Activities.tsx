...
export function Activities({ userId }: VendorProductsProps) {
  ...
  const renderActivities = () => {
    ...
    return productData.data.map((item: any) => {
      if (activityType === 'posts') {
        return <ProductCard key={item.id} userData={userData} productItem={item} view={view} />;
  ...
  <Text color={"#475569"} fontSize={"14px"} pl={"6px"}>
    {` Showing ${productData && productData.items ? productData.items : 0} results`}
  </Text>
</Box>
      ...
      {!isLoading && !productData?.data && "No products found."}
    </>
  );
}